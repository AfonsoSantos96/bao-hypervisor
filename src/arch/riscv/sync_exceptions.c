...
static unsigned long read_ins(virt_addr_t ins_addr)
{
    unsigned long ins = 0;
...
static inline int ins_ldst_decode(virt_addr_t ins, struct emul_access *emul)
{
...
{   
    ...
    virt_addr_t addr = CSRR(CSR_HTVAL) << 2;

    emul_handler_t handler = vm_emul_get_mem(cpu.vcpu->vm, addr);
    if (handler != NULL) {
        virt_addr_t ins_addr = CSRR(sepc);
