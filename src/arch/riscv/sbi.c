...
struct sbiret sbi_hsm_start_handler() {
    ...
    struct sbiret ret;
    cpuid_t vhart_id = vcpu_readreg(cpu.vcpu, REG_A0);
    ...
    if(vhart_id == cpu.vcpu->id){
        ...
        if(vcpu == NULL) {
            ...
            if (vcpu->arch.sbi_ctx.state == STARTED) {
                ...
                unsigned int priv = vcpu_readreg(cpu.vcpu, REG_A2);
...
void sbi_lgcy_rfence_handler(unsigned long extid)
{
    ...
    vm_readmem(cpu.vcpu->vm, &phart_mask, (vaddr_t)hart_mask,
               sizeof(unsigned long), false);
    phart_mask = vm_translate_to_pcpu_mask(cpu.vcpu->vm, (cpumap_t)phart_mask,
                                           sizeof(unsigned long));
