...
void gic_map_mmio()
{
    mem_map_dev(&cpu.as, (virt_addr_t)&gicd, platform.arch.gic.gicd_addr,
    ...
    mem_map_dev(&cpu.as, (virt_addr_t)gicr, platform.arch.gic.gicr_addr, gicr_size);
}
...
void gicr_set_prio(irqid_t int_id, uint8_t prio, uint32_t gicr_id)
{
...
uint64_t gicr_get_prio(irqid_t int_id, uint32_t gicr_id)
{
    uint64_t reg_ind = GIC_PRIO_REG(int_id);
    uint64_t off = GIC_PRIO_OFF(int_id);
    ...
    uint64_t prio =
        gicr[gicr_id].IPRIORITYR[reg_ind] >> off & BIT64_MASK(off, GIC_PRIO_BITS);
...
void gicr_set_icfgr(irqid_t int_id, uint8_t cfg, uint32_t gicr_id)
{
...
void gicr_set_pend(irqid_t int_id, bool pend, uint32_t gicr_id)
{
    spin_lock(&gicr_lock);
    if (pend) {
...
bool gicr_get_pend(irqid_t int_id, uint32_t gicr_id)
{
...
void gicr_set_act(irqid_t int_id, bool act, uint32_t gicr_id)
{
    spin_lock(&gicr_lock);
...
bool gicr_get_act(irqid_t int_id, uint32_t gicr_id)
{
    if (gic_is_priv(int_id)) {
...
void gicr_set_enable(irqid_t int_id, bool en, uint32_t gicr_id)
{
