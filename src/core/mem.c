...
void *mem_alloc_page(size_t n, enum AS_SEC sec, bool phys_aligned)
{
    ...
    if (ppages.size == n) {
        vpage = mem_alloc_map(&cpu()->as, sec, &ppages, NULL_VA, n, 
                                PTE_HYP_FLAGS);
    }
...
bool root_pool_set_up_bitmap(paddr_t load_addr, struct page_pool *root_pool)
{
    ...
    bitmap_t* root_bitmap = (bitmap_t*) 
        mem_alloc_map(&cpu()->as, SEC_HYP_GLOBAL, &bitmap_pp, load_addr, 
                        bitmap_size, PTE_HYP_FLAGS);
...
static void pp_init(struct page_pool *pool, paddr_t base, size_t size)
{
    ...
    if ((pool->bitmap = (bitmap_t*)mem_alloc_map(&cpu()->as, SEC_HYP_GLOBAL, &pages, 
                                    base, bitmap_size, PTE_HYP_FLAGS)) == NULL)
        return;
