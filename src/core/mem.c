...
void color_hypervisor(const paddr_t load_addr, const paddr_t config_addr)
{
    ...
    vaddr_t interface = (vaddr_t) (_cpu_if_base*( PAGE_SIZE * (cpu_new->id)));


    mem_translate(&cpu()->as, (vaddr_t)&interface, &p_intferface_addr);
    p_interface = mem_ppages_get(
        p_intferface_addr,
        NUM_PAGES(sizeof(interface)));
    va = mem_alloc_vpage(
        &cpu_new->as, SEC_HYP_GLOBAL,
        (vaddr_t)&_cpu_if_base + (cpu()->id * sizeof(interface)),
        NUM_PAGES(sizeof(interface)));
    if (va != (vaddr_t)&_cpu_if_base + (cpu()->id * sizeof(interface)))
